...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='INFO', color='[R1]'):
    ...
    def run(self):
        try:
            ...
            self.log('Tunnel Connected [G1]({} port {})'.format(self.proxy_host, self.proxy_port), color='[G1]')
        except OSError:
...
def main():
    print(colors('\n'.join([
        ...
        '[G1](c) 2019 Aztec Rabbit.', '[G1] remode by : Rizred Vlo 7zer0✬', '', '', '[R1]                   ========Inject Sukses======== \nReading Server:', '[CC]'
    ])))
